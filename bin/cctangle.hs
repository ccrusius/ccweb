...
tangleFile opts doc (fp, bs) = do
  let ls = concatMap
           ...
           bs
{-# LINE 415 "ccweb.org" #-}
      ls' = foldr (\x xs -> let a = (x:xs) in case a of
...
{-# LINE 460 "ccweb.org" #-}
  let fs = map (\bs -> (fst (head bs), map snd bs))
             . groupWith fst
             . sortWith fst
             . map (\(fp,b) -> case fp of
                       ('~':xs) -> (home ++ xs, b)
             ...
             . mapMaybe sectionSourceBlock
             $ orgSections org :: [(FilePath, [SourceBlock])]
{-# LINE 446 "ccweb.org" #-}
  logM (logLevel opts) Debug $ "files:" ++ show (map fst fs)
{-# LINE 473 "ccweb.org" #-}
  when (showMakeDep opts) $ putStrLn $ intercalate " " (map fst fs) ++ ": " ++ inputFile opts
